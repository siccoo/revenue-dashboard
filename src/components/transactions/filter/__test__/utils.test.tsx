...
describe('test utility functions', () => {
    ...
    const mockFilterByDaysCriteria: TFilter = {
        days: 'Last 7 days', endDate: '', startDate: '', status: [''], type: ['']
    };
    ...
    const mockFilterByStatusCriteria: TFilter = {
        status: ['Successful'], days: '', endDate: '', startDate: '', type: ['']
    };
    ...
    test('filterByDays filters data correctly for Last 7 days', () => {
        const filteredData = filterByDays(mockTransactionData, mockFilterByDaysCriteria);
        expect(filteredData?.length).toBe(2);
    });

    test('filterByDateRange filters data correctly for the given date range', () => {
        const filteredData = filterByDateRange(mockTransactionData, mockFilterByRangeCriteria);
        expect(filteredData?.length).toBe(3);
    });

    test('filterByType filters data correctly for the given type', () => {
        const filteredData = filterByType(mockTransactionData, mockFilterBytypeCriteria);
        expect(filteredData?.length).toBe(2);
    });
    test('filterByStatus filters data correctly for the given status', () => {
        const filteredData = filterByStatus(mockTransactionData, mockFilterByStatusCriteria);
        expect(filteredData?.length).toBe(3);
    })
